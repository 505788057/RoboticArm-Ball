"""
:Author:  Vitae
:Create:  2020/7/5 11:52
:Methed:  Using the Embedded Systems to process images. Every 50ms figures images out.
...
def myHandler(signum, frame):
    ...
    if new_data == True:
        ...
        MotionPostionMess = IdentifyImage_motion(Image_Part)
        if MotionPostionMess != None:
            MotionPosMessL = list(MotionPostionMess)
            x, y, z = uv2WorldPoint(MotionPosMessL[0], MotionPosMessL[1], MotionPosMessL[2], MotionPosMessL[3])
            xlist.append(x)
            ylist.append(y)
            zlist.append(z)
            print(x, y, z)
            ...
            print('There is no position message.')
        print(time.ctime())
        print(xlist)
        print(ylist)
        print(zlist)
